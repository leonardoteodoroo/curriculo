...
const experience = fs.readFileSync('src/components/sections/Experience.tsx', 'utf8');
const showcase = fs.readFileSync('src/components/sections/ProjectShowcase.tsx', 'utf8');
...
test('project showcase uses placeholders instead of real preview photos', () => {
  ...
  assert.equal(showcase.includes('<img'), false, 'Expected placeholders instead of img tags');
});
